    ...
    public void onServerTick(TickEvent.ServerTickEvent event)
    {
        ...
        if(world.isDaytime())
        {
            ...
            if(isNightfall)
            {
                ...
                GriefDefender.getCore().getClaimManager(((org.spongepowered.api.world.World)world).getUniqueId()).getWildernessClaim().setOption(Options.PVP, "false", Collections.singleton(new Context("plugin", "nightfallpvp")));
                isDaylight = true;
                isNightfall = false;
            ...
            if(isDaylight)
            {
                ...
                GriefDefender.getCore().getClaimManager(((org.spongepowered.api.world.World)world).getUniqueId()).getWildernessClaim().setOption(Options.PVP, "true", Collections.singleton(new Context("plugin", "nightfallpvp")));
                isDaylight = false;
                isNightfall = true;
        ...
        Sponge.getServer().getWorldProperties(Sponge.getServer().getDefaultWorldName()).ifPresent(worldProperties -> {
            ...
            if(worldTime > 13000 && worldTime < 23000)
            {
                ...
                if(isDaylight)
                {
                    ...
                    claim.setOption(Options.PVP, "true", new HashSet<Context>(){{add(new Context("gd_claim", claim.getUniqueId().toString()));}});
                    isDaylight = false;
                    isNightfall = true;
                    logger.info("The night came over, PvP has been activated");
                }
                ...
                if(isNightfall)
                {
                    //it was night before; change
                    Claim claim = GriefDefender.getCore().getClaimManager(worldProperties.getUniqueId()).getWildernessClaim();
                    //claim.setFlagPermission(Flags.ENTITY_DAMAGE, Tristate.FALSE, new HashSet<Context>(){{add(new Context("gd_claim", claim.getUniqueId().toString()));}});
                    claim.setOption(Options.PVP, "false", new HashSet<Context>(){{add(new Context("gd_claim", claim.getUniqueId().toString()));}});
                    isDaylight = true;
                    isNightfall = false;
                    logger.info("The Day rose, PvP has been deactivated");
                }
